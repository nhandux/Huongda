...
$_SESSION["language"] = isset($_GET["lang"]) ? $_GET["lang"] : 'vi';
require_once('..' . DIRECTORY_SEPARATOR . 'define.php');
// Da khac phuc loi ngay 10/3/2016
...
if($type==1) {
	...
	foreach ($images as $image) {
		...
		if ($imgUp->uploaded) {
			...
			//---------
			$imgUp->file_new_name_body      = 'th_' . $name_image;
			...
			if($imgUp->processed) {
				...
				$imgUp->file_new_name_body      = 'list-photo' . $name_image;
				$imgUp->image_resize            = true;
				$imgUp->image_ratio_crop        = true;
				$imgUp->image_y                 = 238;
				$imgUp->image_x                 = 376;
