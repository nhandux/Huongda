		...
		for ($i=0;$i<count($currentdir);$i++) {
			...
			if (!is_dir($entry)) {
				...
				<tr>
					...
					<td align="center">
						<a  data-toggle="tooltip" data-placement="left" title="Tải xuống" href="download.php?<?=TTH_PATH?>=<?=$dir_dest?>&filename=<?=$name?>" ><img src="images/download.png"></a>&nbsp;&nbsp;
						<a  data-toggle="tooltip" data-placement="right" title="Xóa file" class="confirmManager" style="cursor: pointer;" id="?<?=TTH_PATH?>=backup_data&filename=<?=$name?>" ><img src="images/remove.png"></a>
					</td>
				</tr>
			<?
			}
