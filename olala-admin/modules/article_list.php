				...
				<form action="?<?=TTH_PATH?>=article_list" method="post" id="deleteArt">
					...
					<table class="table display table-manager" cellspacing="0" cellpadding="0" id="dataTablesList">
						...
						foreach($rows as $row) {
							...
							<tr>
								...
								<?php
								//-----------Kiểm tra phân quyền ---------------------------------------
								if(in_array("article_edit;".$category_id_core,$corePrivilegeSlug)) {
								?>
								<td align="center">
									<?=($row["is_active"]+0==0)?
					...
					<div class="row">
						...
						<div class="col-sm-6" align="right" style="padding: 7px 0;">
							...
							<input type="button" class="btn btn-primary btn-xs <?=in_array("article_del;".$category_id_core,$corePrivilegeSlug)? "confirmManager" : "alertManager"?> " value="Xóa" name="deleteArt">
						</div>
					</div>
				</form>
			</div>
