				...
				<table class="table table-manager table-hover">
					...
					foreach($rows as $row) {
						...
						<tr class="category">
							...
							<td align="center">
								<a href="?<?=TTH_PATH?>=project_menu_add&id_cat=<?=$row["category_id"]?>"><img data-toggle="tooltip" data-placement="left" title="Thêm mục" src="images/add.png"></a>
								&nbsp;
	...
	foreach($rows2 as $row) {
		...
		<tr>
			...
			if(in_array("project_menu_edit;".$row["category_id"],$corePrivilegeSlug)) {
				?>
				<td align="right">
					<?=showSort("sort_".$row["project_menu_id"]."", $countList,$row["sort"], $width, 0, $row["project_menu_id"] ,'project_menu', 1);?>
				</td>
				<td><?=getNameOthers($row['project_type']);?></td>
				<td align="center">
					<?=($row["is_active"]+0==0)?
			...
			<?php
				...
				?>
				<td align="right">
					<?=showSort("sort_".$row["project_menu_id"]."", $countList,$row["sort"], $width, 0, $row["project_menu_id"] ,'project_menu', 0);?>
				</td>
				<td><?=getNameOthers($row['project_type']);?></td>
				<td align="center">
					<?=($row["is_active"]+0==0)?
			...
			<td align="center">
				...
				if ($level < 30){
					?>
					<a href="?<?=TTH_PATH?>=project_menu_add&id_cat=<?=$row["category_id"]?>&id_art=<?=$row["project_menu_id"]?>"><img data-toggle="tooltip" data-placement="left" title="Thêm mục" src="images/add.png"></a>
					&nbsp;
				<?php } else { ?>
					<span style="width: 16px; height: 1px; display: inline-block;""></span>
					&nbsp;
				<?php } ?>
				<a href="?<?=TTH_PATH?>=project_menu_edit&id=<?=$row["project_menu_id"]?>"><img data-toggle="tooltip" data-placement="top" title="Chỉnh sửa" src="images/edit.png"></a>
				&nbsp;
				<?php
				...
				<?php
					...
					?>
					<a class="confirmManager" style="cursor: pointer;" id="?<?=TTH_PATH?>=project_manager&del=<?=$row["project_menu_id"]?>"><img data-toggle="tooltip" data-placement="right" title="Xóa mục" src="images/remove.png"></a>
				<?php }
				//----------- end if ------------
				?>
			</td>
			<td align="center">
				<a href="?<?=TTH_PATH?>=project_list&id=<?=$row['project_menu_id']?>"><img data-toggle="tooltip" data-placement="top" title="Danh sách bài viết" src="images/list.png"></a>
			</td>
			<?php
			if ($level < 30){
