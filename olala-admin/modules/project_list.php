...
<div class="row">
	<ol class="breadcrumb">
		...
		<li>
			<a href="?<?=TTH_PATH?>=project_manager"><i class="fa fa-edit"></i> Quản lý nội dung</a>
		</li>
		<li>
			<a href="?<?=TTH_PATH?>=project_manager"><i class="fa fa-cubes"></i> Dự án đầu tư</a>
		</li>
		<li>
			<i class="fa fa-list"></i> <?=getNameMenu($project_menu_id, 'project')?>
		</li>
		<a class="btn-add-new" href="?<?=TTH_PATH?>=project_add&id=<?=$project_menu_id?>">Thêm bài viết</a>
	</ol>
</div>
<!-- /.row -->
<?=dashboardCoreAdminTwo("project_list;".$category_id_core)?>
<div class="row">
	<div class="col-lg-12">
		<div class="panel panel-default panel-no-border">
			<div class="table-responsive">
				<form action="?<?=TTH_PATH?>=project_list" method="post" id="deleteArt">
					<input type="hidden" name="list_id" value="<?=$project_menu_id?>" />
					<table class="table display table-manager" cellspacing="0" cellpadding="0" id="dataTablesList">
						...
						foreach($rows as $row) {
							...
							<tr>
								...
								?>
								<td align="center">
									<?=showSort("sort_".$row["project_id"]."", $countList,$row["sort"], "70%", 0, $row["project_id"] ,'project', 1);?>
								</td>
								<td align="center">
									<?=($row["is_active"]+0==0)?
								...
								?>
								<td align="center">
									<?=showSort("sort_".$row["project_id"]."", $countList,$row["sort"], "70%", 0, $row["project_id"] ,'project', 0);?>
								</td>
								<td align="center">
									<?=($row["is_active"]+0==0)?
								...
								<td class="details-control" align="center">
									<div class="checkbox">
										...
										<label class="checkbox-inline">
											<input type="checkbox" data-toggle="tooltip" data-placement="top" title="Xóa" class="checkboxArt" name="idDel[]" value="<?=$row['project_id']?>">
										</label>
									</div>
								</td>
							</tr>
					...
					<div class="row">
						<div class="col-sm-6"><?=showPageNavigation($page, $totalpages,'?'.TTH_PATH.'=project_list&id='.$project_menu_id.'&page=')?></div>
						<div class="col-sm-6" align="right" style="padding: 7px 0;">
							...
							<input type="button" class="btn btn-primary btn-xs <?=in_array("project_del;".$category_id_core,$corePrivilegeSlug)? "confirmManager" : "alertManager"?> " value="Xóa" name="deleteArt">
						</div>
					</div>
				</form>
			</div>
