...
//
function articleMenu($act, $typeFunc, $article_menu_id, $category_id, $name, $slug, $title, $description, $keywords, $parent, $comment, $font_icon, $is_active, $hot, $img, $error) {
	global $tth;
					...
					<table class="table table-hover" style="width: 70%;">
						...
						<tr>
							...
							<td class="element-relative">
								...
								<div data-toggle="tooltip" data-placement="top" title="Tạo liên kết tĩnh" class="btn-get-slug" onclick="return getSlug('article')"></div>
							</td>
						</tr>
						<tr>
						...
						<tr>
							<td colspan="2" align="center">
								...
								<button type="button" class="btn btn-form-info btn-form" onclick="location.href='?<?=TTH_PATH?>=article_manager'">Thoát</button>
							</td>
						</tr>
					</table>
				</form>
...
<script>
	$('.file-img').fileinput({
		<?php if($img!='no' && $img!='') { ?>
		initialPreview: [
			"<img src='../uploads/article_menu/<?=$img?>' class='file-preview-image' title='<?=$img?>' alt='<?=$img?>'>"
		],
		<?php } ?>
