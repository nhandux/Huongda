...
//
function productMenu($act, $typeFunc, $product_menu_id, $category_id, $name, $slug, $title, $description, $keywords, $parent, $is_active, $hot, $img, $error) {
	global $tth;
					...
					<table class="table table-hover" style="width: 70%;">
						...
						<tr>
							<td colspan="2" align="center">
								...
								<button type="button" class="btn btn-form-info btn-form" onclick="location.href='?<?=TTH_PATH?>=product_manager'">Thoát</button>
							</td>
						</tr>
					</table>
				</form>
...
<script>
	$('.file-img').fileinput({
		<?php if($img!='no' && $img!='') { ?>
		initialPreview: [
			"<img src='../uploads/product_menu/<?=$img?>' class='file-preview-image' title='<?=$img?>' alt='<?=$img?>'>"
		],
		<?php } ?>
