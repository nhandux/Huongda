...
//
function tourMenu($act, $typeFunc, $tour_menu_id, $category_id, $name, $slug, $title, $description, $keywords, $parent, $is_active, $hot, $img, $error) {
	global $tth;
	...
	<div class="col-lg-12">
		<div class="panel panel-default">
			<div class="panel-heading">
				<i class="fa fa-files-o"></i> Chuyên mục du lịch
			</div>
			<div class="panel-body">
				<div class="table-respon">
					<form action="<?=$act?>" method="post" enctype="multipart/form-data">
						...
						<input type="hidden" name="tour_menu_id" value="<?=$tour_menu_id?>" />
						...
						<table class="table table-hover" style="width: 70%;">
							...
							<tr>
								...
								<td class="element-relative">
									...
									<div data-toggle="tooltip" data-placement="top" title="Tạo liên kết tĩnh" class="btn-get-slug" onclick="return getSlug('tour')"></div>
								</td>
							</tr>
							<tr>
							...
							<tr>
								<td colspan="2" align="center">
									...
									<button type="button" class="btn btn-form-info btn-form" onclick="location.href='?<?=TTH_PATH?>=tour_manager'">Thoát</button>
								</td>
							</tr>
						</table>
					</form>
...
<script>
	$('.file-img').fileinput({
		<?php if($img!='no' && $img!='') { ?>
		initialPreview: [
			"<img src='../uploads/tour_menu/<?=$img?>' class='file-preview-image' title='<?=$img?>' alt='<?=$img?>'>"
		],
		<?php } ?>
...
function loadMenuCategory($db, $level, $parent, $category_id, $par){
	...
	foreach($rows2 as $row) {
		if ($level <= 2){
			echo "<option value='".$row["category_id"]."'";
			if ($par==$row["tour_menu_id"]) echo " selected ";
			echo ">".stripslashes($row["name"])."</option>";
				loadMenuCategory($db, $level+2, $row["tour_menu_id"]+0, $row["category_id"]+0, $par);
		}
	}
}
