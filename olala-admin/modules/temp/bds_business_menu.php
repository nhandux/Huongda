...
//
function bds_businessMenu($act, $typeFunc, $bds_business_menu_id, $category_id, $name, $slug, $title, $description, $keywords, $parent, $is_active, $hot, $img, $error) {
	global $tth;
					...
					<table class="table table-hover" style="width: 70%;">
						...
						<tr>
							...
							<td class="element-relative">
								...
								<div data-toggle="tooltip" data-placement="top" title="Tạo liên kết tĩnh" class="btn-get-slug" onclick="return getSlug('bds_business')"></div>
							</td>
						</tr>
						<tr>
						...
						<tr>
							<td colspan="2" align="center">
								...
								<button type="button" class="btn btn-form-info btn-form" onclick="location.href='?<?=TTH_PATH?>=bds_business_manager'">Thoát</button>
							</td>
						</tr>
					</table>
				</form>
...
<script>
	$('.file-img').fileinput({
		<?php if($img!='no' && $img!='') { ?>
		initialPreview: [
			"<img src='../uploads/bds_business_menu/<?=$img?>' class='file-preview-image' title='<?=$img?>' alt='<?=$img?>'>"
		],
		<?php } ?>
...
function loadMenuCategory($db, $level, $parent, $category_id, $par){
	...
	foreach($rows2 as $row) {
		if ($level <= 3){
			echo "<option value='".$row["category_id"]."'";
			if ($par==$row["bds_business_menu_id"]) echo " selected ";
			echo ">".stripslashes($row["name"])."</option>";
				loadMenuCategory($db, $level+2, $row["bds_business_menu_id"]+0, $row["category_id"]+0, $par);
		}
	}
}
