				...
				<form action="<?=$act?>" method="post" enctype="multipart/form-data">
					...
					<table class="table table-hover">
						<tr>
							...
							<td width="90%" colspan="3"><input class="form-control" type="text" name="name" maxlength="200" value="<?=stripslashes($name)?>" required="required" ></td>
						</tr>
						<tr>
							<td><label>Mục:</label></td>
							<td colspan="3"><?=categoryName($car_menu_id);?></td>
						</tr>
						<tr>
							...
							<td width="40%" class="ver-top"><input class="form-control" type="text" name="img_note" maxlength="255" value="<?=stripslashes($img_note)?>" ></td>
						</tr>
						<tr>
							...
							<td><input class="form-control" type="text" name="year" maxlength="255" value="<?=stripslashes($year)?>"></td>
						</tr>
						<tr>
							<td><label>Kiểu xe:</label></td>
							<td><input class="form-control" type="text" name="seat" maxlength="255" value="<?=stripslashes($seat)?>"></td>
							<td><label>Màu xe:</label></td>
							<td><input class="form-control" type="text" name="color" maxlength="255" value="<?=stripslashes($color)?>"></td>
						</tr>
						<tr>
							<td><label>Giá thuê:</label></td>
							<td><input class="form-control auto-number" type="text" name="price" maxlength="15" value="<?=stripslashes($price)?>" data-a-sep="." data-a-dec="," data-v-max="9999999999" data-v-min="0" autocomplete="off"></td>
						...
						<tr>
							...
							<td colspan="3"><input class="form-control" id="input-datetime" type="text" name="created_time" style="width: 160px;"  value="<?=$created_time?>" ></td>
						</tr>
						<tr>
						...
						<tr>
							<td colspan="4" align="center">
								...
								<button type="button" class="btn btn-form-info btn-form" onclick="location.href='?<?=TTH_PATH?>=car_list&id=<?=$car_menu_id?>'">Thoát</button>
							</td>
						</tr>
					</table>
				</form>
...
<script>
	...
	$('.file-img').fileinput({
		<?php if($img!='no' && $img!='') { ?>
		initialPreview: [
			"<img src='../uploads/car/<?=$img?>' class='file-preview-image' title='<?=$img?>' alt='<?=$img?>'>"
		],
		<?php } ?>
