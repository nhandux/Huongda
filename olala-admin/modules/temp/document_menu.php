...
//
function documentMenu($act, $typeFunc, $document_menu_id, $category_id, $name, $slug, $title, $description, $keywords, $parent, $comment, $is_active, $hot, $img, $error) {
	global $tth;
		...
		<div class="panel panel-default">
			...
			<div class="panel-body">
				<form action="<?=$act?>" method="post" enctype="multipart/form-data">
					...
					<input type="hidden" name="document_menu_id" value="<?=$document_menu_id?>" />
					...
					<table class="table table-hover" style="width: 70%;">
						<tr>
							...
							<td><input class="form-control" type="text" id="name" name="name" maxlength="200" value="<?=stripslashes($name)?>" required="required" ></td>
						</tr>
						<tr>
							...
							<td class="element-relative">
								...
								<div data-toggle="tooltip" data-placement="top" title="Tạo liên kết tĩnh" class="btn-get-slug" onclick="return getSlug('document')"></div>
							</td>
						</tr>
						<tr>
						...
						<tr>
							...
							<td>
								<textarea class="form-control" name="comment" rows="3"><?=stripslashes($comment)?></textarea>
							</td>
						</tr>
						<tr>
						...
						<tr>
							<td colspan="2" align="center">
								...
								<button type="button" class="btn btn-form-info btn-form" onclick="location.href='?<?=TTH_PATH?>=document_manager'">Thoát</button>
							</td>
						</tr>
					</table>
				</form>
...
<script>
	$('.file-img').fileinput({
		<?php if($img!='no' && $img!='') { ?>
		initialPreview: [
			"<img src='../uploads/document_menu/<?=$img?>' class='file-preview-image' title='<?=$img?>' alt='<?=$img?>'>"
		],
		<?php } ?>
...
function loadMenuCategory($db, $level, $parent, $category_id, $par){
	...
	foreach($rows2 as $row) {
		if ($level <= 3){
			echo "<option value='".$row["category_id"]."'";
			if ($par==$row["document_menu_id"]) echo " selected ";
			echo ">".stripslashes($row["name"])."</option>";
				loadMenuCategory($db, $level+2, $row["document_menu_id"]+0, $row["category_id"]+0, $par);
		}
	}
}
